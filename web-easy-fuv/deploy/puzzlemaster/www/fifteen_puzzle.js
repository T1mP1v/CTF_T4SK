...
function check_slots(){
    ...
    for(var y=0;y<=p.grid[1];y++){
        ...
        setTimeout(() => {
            ...
            form.classList.remove('hidden');  // Убираем класс hidden (если форма была скрыта)
            form.classList.add('show');       // Добавляем класс show для плавного появления
        }, ((p.time) ? p.time * 1000 : 0));
    } // <-- script that runs at the end of the game
}
